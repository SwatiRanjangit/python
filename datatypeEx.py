...
# case-sensitive
# should not start with digits , but it is allowed
# if an identifier start with _(underscore) then it is protected and if there is two underscore at the starting then it is private.
...
# x,y,z = 10 , 20 , 30
# print(x,y,z,sep=":")
# another way of printing the multiple assigned values using end attribute it will avoid line jump and will print in the same line
# print(x,end=" ")
# print(y,end=" ")
# print(z)
...
# DATATYPE:
# python is a dynamic typed programming language means we do not have to specify the data type
# at the time of variables it will be dynamically converted by the python
# datatype represent the type of data we are going to store inside the variables
# Basic datatype
# 1. None = nothing or may be null if we don't want to give any value to the variable at that time we use this if that time we don't hve clarity to assign varible.
# v = None
# print(type(v))
...
# ================== DIR =======================
# - it will print all the realted functions of the object passed into it
...
# =================1.STRING=====================
# in python str represent string can be created by '' or "" (recommended) or
# ''' ''' or """ """ => tripe quotes can be helpful in case of multiple line string or a paragraphn not in case of single word
# strings are immutable can't be modified directly
# "" are recommended generally because suppose if we are assining value like durga's software to a string variable
# we can't able to assign it using '' (single quotes) as compiler got confused about single quotes and apostrophy (') int the string
# and will throw syntax error so we should use ""(double quotes)
# n = 'durga's'software'  syntax error
# in this case to diffrentiate it we have to use \'
# n = 'durag\'s software'
#  so better to use ""
# n = "durag's software"

# TO prove string is immutable as we can take one example
# s = "python is a easy language"
# print("Before replacement: ",s)
# s.replace("is","is not")
# print("After replacement: ",s)   # It will not print the modified string as when we use replace function
                                 # it will return a new string after modification so we need to store the newly created string if we want to print or use it
                                 # that's why we are printing s then it is printing the old value of string


# m = s.replace("is", "is not")
# print("After replacement: ", m)
#  or also we can do like this
# print(s.replace("is", "is not"))   # this is only used to print the replaced string but we can not reuse this modified string
                                     # actually here it basically printing the value returned by replace function
...
# * it is created using {}
# * allow diffrent data type (heterogeneous) collections of elements
# * it is mutable
...
# d = {1:"durga", 2: "swati", 3: "ram",4:"durga"}
# d = {1:"durga", "name" : "swati", 3.0:10,4:"durga"}
# d={(1,2,3): "swati"} # key is of tuple type as tuple is immutable so it can be used as key as key must be immutable or unique
# print(d)

# # NOTE: here as a key we can't take list and set as list is mutable but as per key rule key can not be mutable so it will give error if we use it
# d={[10,20]:"swati"}
# print(d)
